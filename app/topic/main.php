	...
	public function index_action()
	{
		...
		switch ($topic_info['model_type'])
		{
			default:
				...
				$task_list = $this->model('task')->get_task_list_by_topic_ids(1, get_setting('contents_per_page'), 'time DESC', $contents_topic_id);
				...
				if ($task_list)
				{
					...
					{
						$task_ids[] = $val['taskid'];
						$task_list[$key]['user_info'] = $this->model('account')->get_user_info_by_uid($val['uid'], true);
					}
					...
					foreach ($task_list AS $key => $val)
					{
						$task_list[$key]['topics'] = $task_topics[$val['taskid']];
					}
				}
				
				TPL::assign('task_list', $task_list);
				TPL::assign('task_list_bit', TPL::output('task/topic_task_ajax', false));
