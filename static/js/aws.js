...
AWS.Init =
{
	...
	// 初始化帖子评论回复框
	init_thread_comment_box: function(selector)
	{
	    $(document).on('click', selector, function ()
	    {
	    	var _editor_box = $(this).parents('.aw-item').find('.aw-article-replay-box');
	        ...
	        {
	            $(this).parents('.mod-footer').append(Hogan.compile(AW_TEMPLATE.threadCommentBox).render(
	            {
	                'at_uid' : $(this).attr('data-id'),
					'commentid' : $(this).attr('comment-id'),
	                'threadid' : $('.aw-topic-bar').attr('data-id')
	            }));
	        }
	    });
	},
